...
class PlottarGraficos(ManipulationPandas):
    ...
    def plottar(self,
        ...
        if nomeArquivo.endswith('nginx_response.csv'):
            if multiply != 1:
                df['response_time'] = df['response_time'] * multiply
            
            else:
                df['response_time'] = df['response_time'] / division
        ...
        for col in df.columns:          
            ...
            if col == 'swap-free':
                # Forçar faixa de exibição no eixo Y
                ax.set_ylim([970, 980])  # Ajuste conforme necessidade

                # Desabilitar notação científica e formatar
